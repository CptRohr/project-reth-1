...
func _show_calendar_panel() -> void:
	...
	calendar_view_month = int(
		get_calendar_manager().get_date_info(
			GameState.calendar_day_index
		)["month"]
	)
...
func _update_clock() -> void:
	...
	var hour_fraction: float = (
		float(hour % 12)
		+ float(minute) / 60.0
		+ float(second) / 3600.0
	)

	var minute_fraction: float = (
		float(minute)
		+ float(second) / 60.0
	)

	clock.set_hour_angle(
		hour_fraction * TAU / 12.0 - PI / 2.0
	)

	clock.set_minute_angle(
		minute_fraction * TAU / 60.0 - PI / 2.0
	)
...
func _update_selector_hand() -> void:
	...
	var clock_center: Vector2 = (
		clock.global_position + clock.size * 0.5
	)

	var target_center: Vector2 = (
		target.global_position + target.size * 0.5
	)

	clock.set_selector_angle(
		(target_center - clock_center).angle()
	)
...
func _setup_settings_values() -> void:
	...
	resolution_options.select(
		resolution_index if resolution_index != -1 else 0
	)
	...
	master_volume_slider.value = AudioSettings.get_volume_percent(
		AudioSettings.MASTER_BUS
	)

	sfx_volume_slider.value = AudioSettings.get_volume_percent(
		AudioSettings.SFX_BUS
	)

	music_volume_slider.value = AudioSettings.get_volume_percent(
...
func _apply_settings() -> void:
	if fullscreen_check.button_pressed:
		...
		DisplayServer.window_set_size(
			RESOLUTIONS[resolution_options.selected]
		)
	...
	AudioSettings.set_volume_percent(
		AudioSettings.MASTER_BUS,
...
func _update_audio_volume_labels() -> void:
	master_volume_value.text = str(
		int(master_volume_slider.value)
	)

	sfx_volume_value.text = str(
		int(sfx_volume_slider.value)
	)

	music_volume_value.text = str(
		int(music_volume_slider.value)
	)
...
func _get_wrapped_month(direction: int) -> int:
	var story_months: Array = (
		get_calendar_manager().get_story_months()
	)

	var month_index := story_months.find(calendar_view_month)
	...
	return int(
		story_months[
			(month_index + direction + story_months.size())
			% story_months.size()
		]
	)
...
func _refresh_calendar_view() -> void:
	...
	var view_info = calendar_manager.get_date_info(
		calendar_manager.parse_date_key(
			"%02d/01" % calendar_view_month
		)
	)

	calendar_month_label.text = "%s %s" % [
		view_info["month_name"],
		calendar_manager.STORY_YEAR
	]
	...
	for cell in calendar_manager.get_month_grid(
		calendar_view_month,
		GameState.calendar_day_index
	):
		var day_label := Label.new()
		day_label.custom_minimum_size = Vector2(90, 60)
...
func _is_non_pause_scene() -> bool:
	...
	return (
		...
		and [
			MAIN_MENU_SCENE,
			OPENING_CUTSCENE_SCENE
		].has(current_scene.scene_file_path)
	)
